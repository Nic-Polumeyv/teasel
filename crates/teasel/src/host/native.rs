	...
	pub(super) fn native_construct(
		...
		Ok(match ty {
			...
			"js.Property" => NodeKind::Property {
				...
				computed: get(Key::Computed).yes(),
				method: get(Key::Method).yes(),
				shorthand: get(Key::Shorthand).yes(),
			},
			"js.SpreadElement" => NodeKind::SpreadElement {
				argument: node(Key::Argument)?,
			},
			"js.UnaryExpression" => NodeKind::UnaryExpression {
				operator: match self.text_of(get(Key::Operator)).unwrap_or("") {
			...
			"js.ConditionalExpression" => NodeKind::ConditionalExpression {
				test: node(Key::Test)?,
				consequent: node(Key::Consequent)?,
				alternate: node(Key::Alternate)?,
			},
			"js.MemberExpression" => NodeKind::MemberExpression {
				object: node(Key::Object)?,
				property: node(Key::Property)?,
				computed: get(Key::Computed).yes(),
				optional: get(Key::Optional).yes(),
			},
			"js.CallExpression" => NodeKind::CallExpression {
				callee: node(Key::Callee)?,
				arguments: list(self, Key::Arguments)?,
				optional: get(Key::Optional).yes(),
			},
			...
			"js.ArrowFunctionExpression" => NodeKind::ArrowFunctionExpression {
				params: list(self, Key::Params)?,
				body: node(Key::Body)?,
				expression: get(Key::Expression).yes(),
				is_async: get(Key::Async).yes(),
			},
			"js.ClassBody" => NodeKind::ClassBody {
				body: list(self, Key::Body)?,
			},
			...
			"js.YieldExpression" => NodeKind::YieldExpression {
				argument: optional(Key::Argument),
				delegate: get(Key::Delegate).yes(),
			},
			...
			"js.ExpressionStatement" => NodeKind::ExpressionStatement {
				expression: node(Key::Expression)?,
				directive: if matches!(get(Key::Directive), Datum::Missing | Datum::Null) {
					None
				} else {
					Some(string(self, Key::Directive)?)
				},
			},
			...
			"js.IfStatement" => NodeKind::IfStatement {
				test: node(Key::Test)?,
				consequent: node(Key::Consequent)?,
				alternate: optional(Key::Alternate),
			},
			"js.SwitchStatement" => NodeKind::SwitchStatement {
				discriminant: node(Key::Discriminant)?,
				cases: list(self, Key::Cases)?,
			},
			"js.SwitchCase" => NodeKind::SwitchCase {
				test: optional(Key::Test),
				consequent: list(self, Key::Consequent)?,
			},
			"js.ThrowStatement" => NodeKind::ThrowStatement {
				argument: node(Key::Argument)?,
			},
			"js.TryStatement" => NodeKind::TryStatement {
				block: node(Key::Block)?,
				handler: optional(Key::Handler),
				finalizer: optional(Key::Finalizer),
			},
			"js.CatchClause" => NodeKind::CatchClause {
				param: optional(Key::Param),
				body: node(Key::Body)?,
			},
			...
			"js.ForStatement" => NodeKind::ForStatement {
				init: optional(Key::Init),
				test: optional(Key::Test),
				update: optional(Key::Update),
				body: node(Key::Body)?,
			},
			...
			"js.ForOfStatement" => NodeKind::ForOfStatement {
				...
				right: node(Key::Right)?,
				body: node(Key::Body)?,
				is_await: get(Key::IsAwait).yes(),
			},
			"js.VariableDeclaration" => NodeKind::VariableDeclaration {
				declarations: list(self, Key::Declarations)?,
				kind: match self.text_of(get(Key::Kind)).unwrap_or("") {
			...
			"js.ImportDeclaration" => NodeKind::ImportDeclaration {
				specifiers: list(self, Key::Specifiers)?,
				source: node(Key::Source)?,
				attributes: list(self, Key::Attributes)?,
			},
			"js.ImportSpecifier" => NodeKind::ImportSpecifier {
				imported: node(Key::Imported)?,
				local: node(Key::Local)?,
			},
			"js.ImportDefaultSpecifier" => NodeKind::ImportDefaultSpecifier {
				local: node(Key::Local)?,
			},
			"js.ImportNamespaceSpecifier" => NodeKind::ImportNamespaceSpecifier {
				local: node(Key::Local)?,
			},
			...
			"js.FunctionExpression" | "js.FunctionDeclaration" => {
				let function = crate::ast::Function {
					...
					params: list(self, Key::Params)?,
					body: node(Key::Body)?,
					generator: get(Key::Generator).yes(),
					is_async: get(Key::Async).yes(),
				};
			...
			"js.ClassExpression" | "js.ClassDeclaration" => {
				let class = crate::ast::Class {
					id: optional(Key::Id),
					super_class: optional(Key::SuperClass),
					body: node(Key::Body)?,
				};
			...
			"js.ExportNamedDeclaration" => {
				if let Some(declaration) = optional(Key::Declaration) {
					NodeKind::ExportDeclaration { declaration }
				} else {
					NodeKind::ExportNamedDeclaration {
						specifiers: list(self, Key::Specifiers)?,
						source: optional(Key::Source),
						attributes: list(self, Key::Attributes)?,
					}
				}
			}
			"js.TemplateElement" => {
				let value = get(Key::Value);
				...
				NodeKind::TemplateElement {
					raw: self.intern(&raw),
					cooked,
					tail: get(Key::Tail).yes(),
				}
			}
			"js.Literal" => {
				if !matches!(get(Key::Regex), Datum::Missing | Datum::Null) {
					let value = get(Key::Regex);
				...
				} else if !matches!(get(Key::Bigint), Datum::Missing | Datum::Null) {
					if start == end
						|| !self
							.src
