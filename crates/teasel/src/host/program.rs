...
impl Builder {
	...
	fn name(&mut self, name: &str) -> StrId {
		if let Some(i) = self.strings.iter().position(|s| s.as_ref() == name) {
			return StrId::at(i as u32);
		}
		let id = StrId::at(self.strings.len() as u32);
		...
		match expr {
				...
				let base = match base {
					plan::Base::Value(value) => {
						...
						if let ExprTree::Get { base, path } = value {
							...
							{
								let slot = self.slot(&self.program.strings[p.name.index() as usize]);
								path.remove(0);
			...
			V::Compare {
				...
				if let Some(set) = set {
					...
					strings.sort_unstable_by_key(|id| id.index());
		...
		match self {
			...
			Self::Member { needle, strings } => {
				...
				if let Self::Constant(Datum::Text(id)) = needle.as_ref() {
					*self = Self::Constant(Datum::Bool(
						strings.binary_search_by_key(&id.index(), |id| id.index()).is_ok(),
					));
				}
			}
...
impl Symbol {
	pub(super) fn id(self) -> StrId {
		StrId::at(self as u32)
	}
}
...
impl Program {
	...
	pub(super) fn lower(plan: &plan::Plan) -> Self {
		...
		for id in 0..p.strings.len() {
			let range = p.dispatch_for(Some(StrId::at(id as u32)));
		...
		let mut name = |text: &str| match p.strings.iter().position(|s| s.as_ref() == text) {
			Some(i) => StrId::at(i as u32),
			None => {
				p.strings.push(text.into());
				StrId::at(p.strings.len() as u32 - 1)
			}
		};
		p.keys = Keys {
			start: name("start"),
