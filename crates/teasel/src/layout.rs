//! The tree's memory layout, told to a front end that reads the nodes in place: every kind's
//! fields by byte offset and type, and how the compiler spells a missing optional.
...
fn field(out: &mut String, field: &Field) {
	...
	out.push_str("\",\"at\":");
	crate::estree::push_int(out, field.at as u32);
	out.push_str(",\"ty\":\"");
...
fn list(out: &mut String, fields: &[Field]) {
	out.push('[');
...
fn variants(out: &mut String, variants: &[Variant]) {
	out.push('[');
	for (i, variant) in variants.iter().enumerate() {
		...
		out.push_str("\",\"fields\":");
		list(out, variant.fields);
		out.push('}');
	}
	out.push(']');
}

/// The layout as JSON: the node record's size and where its span sits, the kinds in tag order
/// with their fields, the TypeScript kinds and the extras record when built with them, and the
/// spellings of a missing optional.
pub fn json() -> String {
	use crate::ast::Node;
	let mut out = String::new();
	out.push_str("{\"node\":{\"size\":");
	crate::estree::push_int(&mut out, std::mem::size_of::<Node>() as u32);
	out.push_str(",\"start\":");
	...
	{
		use crate::typescript::ast::{Extras, TsKind};
		out.push_str(",\"ts\":{\"size\":");
		crate::estree::push_int(&mut out, std::mem::size_of::<TsKind>() as u32);
		out.push_str(",\"kinds\":");
