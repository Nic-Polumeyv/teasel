...
`scopes: true` adds scope analysis to an answer: every node that opens a scope carries `scope`, every identifier that declares or refers to something carries `binding` (null for a global) with `write` when it is assigned to and `mutate` when a member of its value is, and the answer lists `scopes` and `bindings`, linked into objects by the bindings so `binding.references`, `binding.node`, `scope.declarations` and `scope.through` (the outer bindings a scope's code reaches) are there to read. The rules are ECMAScript's: `var` and function declarations belong to the nearest function, program or static block; `let`, `const`, classes, catch parameters and loop heads to their block; a named function expression and a named class expression bind their own name inside, while a class declaration's body sees the declaration's binding; `arguments` is a binding of the function that reads it; references resolve when their scope closes, so use before declaration resolves. TypeScript type positions bind nothing, not even a `typeof x` query, while enums, namespaces, `import =` and ambient functions declare values. `oracle/scopes.js` diffs every resolution and every write against eslint-scope over acorn's tree.
