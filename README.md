...
A JavaScript and TypeScript parser in Rust. It produces acorn's ESTree, byte for byte, and fixes what makes acorn awkward under a compiler that embeds JavaScript in a larger syntax: one parse reads a program, an expression, a pattern, a parameter list, a statement or a type parameter list at an offset in a bigger source, attaches comments, analyzes scopes, and reads TypeScript without a plugin. Svelte is the first consumer; nothing in it is specific to Svelte. The npm name `@teasel/parser` is reserved for it.
...
`parser::parse_at` is the one parse: an entry (program, expression, pattern, params, statement or type parameters), an offset, where the source is cut, the options and the host's stop tokens go in, and the tree, its roots and the offset after what was consumed come out. `json::parse` wraps it for every front end: a request names what to parse, where, and how, and the answer is `{ node, end }` with what the options add, as JSON or as a token stream, or an error with a code, a message, a span and a location. The CLI, the Node addon in `bindings/node` and the WebAssembly module in `bindings/wasm` all go through it.

`@teasel/parser` is the Node package: `new Source(text, options)` keeps a source with its options, acorn's with acorn's defaults (`sourceType: 'script'`, no `loc` without `locations`) plus `typescript`, `comments`, `scopes`, `parenthesized` and `errorRecovery`, so the parses out of it share one copy and one set of position tables, and `source.parse(entry, offset, { end, stopAt })` reads one entry of it: `'program'`, the default, is the whole source or the range `offset..end` of it, a script in a template, with the positions of the whole; `'expression'`, `'pattern'`, `'params'`, `'statement'` and, for TypeScript, `'typeParameters'` read the host's embedded JavaScript at `offset`. Every answer is `{ node, end }` and what the options add: `node` is the tree, or the patterns of a parameter list, and `end` is the offset after everything the parse consumed, closing parens and trailing comments included, so a host knows where its own syntax resumes; `comments` lists every comment read when the option is on. A syntax error throws a `SyntaxError` whose `code` names what went wrong (`unexpected_token`, `unexpected_eof`, `unterminated_regexp`, ...), with `pos` and `end` spanning the offending token and a `loc`; the message says it in words and carries no position. `stopAt` lists the host's own tokens, words or punctuators, that follow what is parsed: one read outside every bracket the parse opened ends it whatever else it could have been, so `{#each list as item}` ends before `as` even in TypeScript, where an assertion in the list needs parens, `then` after `.` is not a property name, `,` ends the expression before a sequence would, and `/>` is not a division. `errorRecovery` records syntax errors as `errors` on the answer instead of throwing the first: an `Identifier` named `''` of no width stands where an operand, name or pattern is missing, and a statement or entry that cannot be read is skipped to the next stop token or unmatched closing bracket, an empty identifier standing for it; a parse can never grow past what its source holds, so a defect in recovery is an error, not a runaway. `parenthesized: true` marks a node the source wraps in parens, on the node itself and absent otherwise; there is no wrapper node. `parentOf(node)` is the node a node sits in, so a host walks up without keeping paths of its own; `isIdentifierStart` and `isIdentifierChar` are exported for hosts that tokenize around embedded JavaScript, and `source.free()` releases what the engine holds for it before the collector would. `typescript: 'erase'` parses TypeScript and emits JavaScript: annotations, type-only declarations and imports go, assertions give way to their expression, and what erasure cannot express (enums, namespaces with values, parameter properties, `export =`, `import =`) stays in the tree and is listed as `typescript` on the answer, as are the proposals JavaScript itself has, decorators and accessor fields. Offsets are UTF-16, as in acorn; a lone surrogate inside a string literal reaches the parser as U+FFFD. The package is `package`: `bun run build` there builds the Node addon in `bindings/node` and `bun run build:wasm` the WebAssembly module in `bindings/wasm`, and one `api.js` gives both the same API over an engine of five calls. Node resolves the package to the addon and everything else to `wasm.js`, which loads `teasel.wasm` from next to itself as it is imported and needs no glue and no dependency: the source is written into the module's memory once and every answer is read straight out of it. Every key the options add to an answer is there exactly when its option is on.

The Node package does not go through JSON. The writer that produces acorn's JSON for the CLI writes, behind the same `Sink`, a shape-coded stream (`estree::Binary`): one buffer of 32-bit words where a node is the id of its shape, the type and the keys it holds with the kind of each value, numbered once per thread as the constants are, followed by its values with no keys and no tags between them; the tree's strings follow as UTF-8 with their UTF-16 ends, source slices are offsets. `decode.js` generates one builder per shape, an object literal V8 allocates in one hidden class, and turns the stream into ESTree objects at a twelfth of what `JSON.parse` costs, or interprets the shapes where a host forbids code generation; the addon takes the source as the UTF-8 bytes V8 encodes, one copy into Rust, and answers with a view of a buffer it keeps, so the hand-over back allocates and copies nothing. On a 66 KB file under Node, best of repeated runs on one pinned core, acorn parses in 1.3 ms with `locations` and 1.1 ms without; teasel end to end takes 0.8 ms and 0.6 ms. `check.js` in `package` parses every script under a directory three ways and diffs the trees: the addon's decoded against the JSON the binary prints, and the wasm module's against the addon's. The wasm module answers the same stream, its words read in place from the module's memory and its answer buffer reused from one parse to the next.

## Scopes

`scopes: true` adds scope analysis to an answer: it lists `scopes`, `bindings` and `references`, and the package answers `scopeOf(node)`, the scope a node opens, and `bindingOf(identifier)`, what it declares or refers to (null for a global, undefined when it names no value), and `referenceOf(identifier)` for a reference's `write`, an assignment or update, and `mutate`, one to a member of its value, which a global's still has; the tree itself stays plain ESTree, and a copy of a node carries no facts. A binding's `declaration` is the node that declares it and a write's `writeExpr` what it assigns, both as eslint-scope names them, and a program or fragment scope says whether an `await` runs at its top level as `topLevelAwait`, so a host reads declarations and assignments off the answer instead of walking for them. Every reference is a row of the `references` table with its `scope`, `binding`, `write`, `read` (a compound assignment or an update reads too, a plain assignment's target does not) and `mutate`. In JSON the facts sit on the nodes as numbers indexing the tables: `scope`, `declares`, `reference`, `defines` (the bindings a node declares) and `writes` (the references a node is assigned to); `binding.node`, `binding.declaration`, `reference.node` and `reference.writeExpr` are linked by the package, and anything grouped, a scope's bindings say, is one filter over the tables. The rules are ECMAScript's: `var` belongs to the nearest function, program or static block; `let`, `const`, classes, function declarations, catch parameters and loop heads to their block; a named function expression and a named class expression bind their own name inside; a parameter default sees past what the body declares; `arguments` is a binding of the function that reads it; references resolve when their scope closes, so use before declaration resolves. Two deliberate departures: a class declaration's body refers to the declaration's binding rather than to a second, immutable one, since that is the reference a compiler wants; and Annex B's `var`-like hoisting of block functions in sloppy scripts is not modelled. Inside `with`, references resolve statically as elsewhere. TypeScript type positions bind nothing, not even a `typeof x` query, and neither do type-only imports and exports, while enums and their members, namespaces (their blocks sharing one scope), `import =` and function signatures declare values. A pattern parsed on its own declares `pattern` bindings, for the host to say what kind they are. `oracle/scopes.js` diffs every resolution and every write against eslint-scope over acorn's tree: `SVELTE_DIR=~/Projects/svelte bun scopes.js`.
