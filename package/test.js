// bun test.js: the package's tests. `bun test.js interpret` runs the decoder without code generation,
// as a host forbidding it would. `node test.js check DIR...` parses every script under the directories
// three ways and diffs the answers: the addon's decoded stream against the JSON the binary prints,
// and the wasm module's against the addon's; `cargo build --release` first.
...
const { ENTRY, names, decode } = await import('./api.js');
...
if (process.argv[2] === 'check') {
	...
	for (const dir of process.argv.slice(3)) walk(dir);
	let checked = 0;
	let failed = 0;
